...
In order to achieve an optimal startup time when the server was previously built with any **build option**, you should run the `start` command together with the `--no-build` option and including any **configuration option** you want to set.

.Run the `start` command to start the server while setting various database options
<@kc.start parameters="--no-build --db-url-host=keycloak-postgres --db-username=keycloak --db-password=change_me --hostname mykeycloak.acme.com"/>

The `--no-build` indicates that the server was previously configured with any build option so that it no longer need to run the `build` command prior to start the server.
