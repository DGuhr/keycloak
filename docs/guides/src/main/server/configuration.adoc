...
* user-created `keycloak.conf` file
* `keycloak.conf` file located in the `conf` directory.

Configuration sources have a descending ordinal: command-line parameters take precedence over environment variables. Environment variables take precedence over options set by using a specific configuration file. Options from a specific config file take precedence over options defined in `conf/keycloak.con`. When the same configuration key is found in multiple configuration sources, the used value is taken from the Source with the highest ordinal.
...
<@kc.start rootParameters="--config-file=myconfig.conf"/>
...
== Optimise the Keycloak startup
It is highly recommended to optimise Keycloak for better startup times and memory consumption before deploying into production environments. This section shows you how to apply a set of optimisations for Keycloak to get the best performance and runtime behaviour possible.

=== Create an optimised Keycloak build
...
This `build` command performs a set of optimisations to achieve an optimised startup- and runtime-behaviour. The build process can take some time, usually a few seconds. Especially when running Keycloak in containerized environments like Kubernetes or OpenShift, startup time is important.
...
==== Second step: Start Keycloak using `--optimised`
After a successful build, you can start Keycloak and turn off the default startup behaviour by invoking the following command:

<@kc.build parameters="--optimised <configuration-options>"/>

The `--optimised` parameter tells Keycloak to assume a pre-built, already optimised Keycloak image is used. As a result, Keycloak avoid checking for and running a build directly at startup to spare the time to walk through this process.
...
If a build option is found at startup with an equal value to the value used when invoking the `build`, it gets silently ignored when using the `--optimised` flag. If it has a different value than the value used when a build was invoked, a warning is shown in the logs and the previously built value is used. In order for this value to take effect, you have to run a new `build` before starting.

.Example: Create an optimised build, then start Keycloak using the --optimised parameter

1. Set build option for postgres using the build command

    <@kc.build parameters="--db=postgres"/>

2. Set configuration options for postgres inside `conf/keycloak.conf`
...
3. Start the server with the optimised parameter

    <@kc.start parameters="--optimised"/>

Most optimisations to startup and runtime behaviour can be achieved by using the `build` command. By using the `keycloak.conf` file as a source for configuration options, Keycloak avoids some steps at startup that are needed when invoking the configuration using the command line, for example initialising the CLI itself. As a result, the server starts up even faster.

== Underlying concepts
This section gives an overview around the underlying concepts Keycloak uses, especially when it comes to optimising the startup.
...
The following are some optimisations performed by the `build` command:
